...
from shared.metrics import (
    MetricData, DailyMetrics, MonthlyMetrics, SystemHealth, PerformanceMetrics,
    EfficiencyScore, MetricType, get_date_string, get_month_string, 
    calculate_success_rate, calculate_cost_per_request
)
...
async def get_daily_metrics(
    ...
    try:
        ...
        metrics = db.query(MetricRecord).filter(
            ...
            if metric.metric_type == "request":
                ...
                if metric.response_time_ms:
                    daily.avg_response_time_ms = (
                        (daily.avg_response_time_ms * (daily.total_requests - 1) + metric.response_time_ms) 
                ...
                if metric.agent_type:
                    daily.agent_task_count[metric.agent_type] = daily.agent_task_count.get(metric.agent_type, 0) + 1
                ...
                if metric.task_duration_ms:
                    daily.avg_task_duration_ms = (
                        (daily.avg_task_duration_ms * (daily.total_agent_tasks - 1) + metric.task_duration_ms)
                ...
                daily.total_tokens += (metric.input_tokens or 0) + (metric.output_tokens or 0)
                if metric.cost_usd:
                    daily.total_llm_cost += metric.cost_usd
                    if metric.model_name:
                        daily.llm_cost_by_model[metric.model_name] = (
                            daily.llm_cost_by_model.get(metric.model_name, 0) + metric.cost_usd
                        )
        ...
        for agent, count in daily.agent_task_count.items():
            agent_tasks = [m for m in metrics if m.agent_type == agent and m.metric_type == "agent_execution"]
            agent_successful = len([m for m in agent_tasks if m.task_status == "success"])
            daily.agent_success_rate[agent] = calculate_success_rate(agent_successful, count)
...
@app.get("/metrics/monthly/{year_month}", response_model=MonthlyMetrics, tags=["Analytics"])
async def get_monthly_metrics(
    ...
    try:
        ...
        if monthly.total_requests > 0:
            monthly.cost_per_request = calculate_cost_per_request(monthly.total_cost, monthly.total_requests)
        
        # Average metrics
        if len(monthly.daily_metrics) > 0:
            monthly.avg_response_time_ms = sum(d.avg_response_time_ms for d in monthly.daily_metrics) / len(monthly.daily_metrics)
            monthly.avg_agent_task_duration_ms = sum(d.avg_task_duration_ms for d in monthly.daily_metrics) / len(monthly.daily_metrics)
            
            if monthly.total_requests > 0:
                monthly.avg_success_rate = (monthly.total_requests - sum(d.failed_requests for d in monthly.daily_metrics)) / monthly.total_requests * 100
            
            monthly.error_rate = (sum(d.total_errors for d in monthly.daily_metrics) / max(monthly.total_requests, 1)) * 100
            
            # Error breakdown
            for daily in monthly.daily_metrics:
                for error_type, count in daily.error_breakdown.items():
                    monthly.error_types[error_type] = monthly.error_types.get(error_type, 0) + count
        ...
        for model in all_models:
            total_model_cost = sum(d.llm_cost_by_model.get(model, 0) for d in monthly.daily_metrics)
        ...
        for agent in all_agents:
            rates = [d.agent_success_rate.get(agent, 0) for d in monthly.daily_metrics if agent in d.agent_success_rate]
            if rates:
...
async def get_system_health(db: Session = Depends(get_db)):
    ...
    try:
        ...
        recent_errors = db.query(MetricRecord).filter(
...
@app.get("/metrics/efficiency-score", response_model=EfficiencyScore, tags=["Analytics"])
async def get_efficiency_score(days: int = 7, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if requests:
            avg_latency = sum(m.response_time_ms for m in requests if m.response_time_ms) / max(len(requests), 1)
        ...
        if requests and llm_calls:
            ...
            cost_efficiency_score = max(0, 25 * (0.01 - cost_per_req) / 0.01) if cost_per_req < 0.01 else 0
        ...
        total_score = reliability_score + latency_score + cost_efficiency_score + resource_utilization_score
        ...
        if reliability_score < 20:
            score.recommendations.append("Improve task reliability - focus on error handling")
        if latency_score < 20:
            score.recommendations.append("Optimize response times - consider caching strategies")
        if cost_efficiency_score < 20:
            score.recommendations.append("Review LLM usage patterns - consider cheaper models for simple tasks")
